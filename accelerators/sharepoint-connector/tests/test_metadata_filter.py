...
class TestMatchesMetadataFilter:
    ...
    def test_not_equal_excludes_matching_value(self):
        """A `<>` condition fails when the column equals the value."""
        fields = {"FileName": "Main Document.docx"}
        mf = self._mf(("FileName", "<>", "Main Document.docx"))
        assert _matches_metadata_filter(fields, mf) is False

    def test_not_equal_passes_for_different_value(self):
        fields = {"FileName": "Policy.docx"}
        mf = self._mf(("FileName", "<>", "Main Document.docx"))
        assert _matches_metadata_filter(fields, mf) is True

    def test_not_equal_is_case_insensitive(self):
        fields = {"FileName": "MAIN DOCUMENT.DOCX"}
        mf = self._mf(("FileName", "<>", "Main Document.docx"))
        assert _matches_metadata_filter(fields, mf) is False

    def test_equal_and_not_equal_combined(self):
        """Customer scenario: approved docs excluding 'Main Document.docx'."""
        mf = self._mf(("DDStatus", "=", "Approved"), ("FileName", "<>", "Main Document.docx"))
        assert _matches_metadata_filter(
            {"DDStatus": "Approved", "FileName": "Policy.docx"}, mf
        ) is True
        assert _matches_metadata_filter(
            {"DDStatus": "Approved", "FileName": "Main Document.docx"}, mf
        ) is False
        assert _matches_metadata_filter(
            {"DDStatus": "Draft", "FileName": "Policy.docx"}, mf
        ) is False
